...
class Aff2ComAUDataset(Dataset):
    ...
    def audiodata(self, selected_p):
        ...
        # [2,44100]
        audio = audio[:, int(self.sample_rate*start_frame/30)
                             :int(self.sample_rate*(end_frame+2)/30)]
...
class Aff2ComAUDatasetTDNN(Dataset):
    def __init__(self, transform=None, flag='train', withBP4D=False, is_random=False, seq_num=30):
        super(Aff2ComAUDatasetTDNN, self).__init__()
        self.fps_dic = np.load('/data/tian/Data/ABAW2_Competition/video_fps_dic.npy',allow_pickle=True).item()
        self._flag = flag
        self.aus = [1, 2, 4, 6, 7, 10,  12,  15, 23, 24, 25, 26]
        self.au_names = [f'AU{au:0>2}' for au in self.aus]
        self._transform = transform
    ...
    def audiodata(self, selected_p):
        ...
        subject = selected_p[0].split('/')[-2]  # video name
        audio_file = '/data/tian/Data/Aff-Wild2-audio/audio/' + subject + '.wav'
        # fps = self.fps_dic[subject.split('_')[0]]
        if subject not in self.fps_dic:
            if subject.split('_')[-1]=='left' or subject.split('_')[-1]=='right':
                video_name = subject.split('_')[0]
                fps = self.fps_dic[video_name]
            else:
                fps = 30

        else:
                fps = self.fps_dic[subject]
        # print(subject,fps)
        if not os.path.exists(audio_file):
            # print('file not exist:',audio_file)
            audio_file = audio_file.split('_')[0]+'.wav'
        ...
        # [2,44100]
        # audio = audio[:,
        audio = audio[:, int(self.sample_rate*(start_frame-1)/fps)
                             :int(self.sample_rate*(end_frame-1+1)/fps)]
    ...
    def __getitem__(self, index):
        ...
        data['img'] = im_data  # (30,3,112,112)
        data['label'] = im_labels  # (30,12)
        data['audio'], frame_list = self.audiodata(selected_p)  # (30,2,64,8)
        # print(data['audio'].shape)
