...
save_dir = './models_BP4D_withdropout_lianxu/attention_pos_weight2_seq30_premodel/lr0.01'  
...
cuda_num = 7                                                                                                     # only support signle gpu card
...
print_every = 500                                                                                                      # print info every
...
# pretrained_model = torch.load('/data/tian/developer/ROI-Nets-pytorch/models_BP4D/score_0.6696params_epoch_0_acc_0.8740_f1_0.4651.pkl')
pretrained_model = torch.load('/data/tian/developer/output/model_2021-06-17-17-16_acc:0.8787685929713989_f1:0.5328100042602213_bacth:1_step:12053.pth')
...
learning_rate = 0.01
# opt = torch.optim.SGD(filter(lambda p: p.requires_grad, net.parameters()),lr=learning_rate,momentum=0.9,nesterov=True,weight_decay=5e-5)
opt = torch.optim.SGD(filter(lambda p: p.requires_grad, net.parameters()),lr=learning_rate,momentum=0.9,weight_decay=5e-5)
...
train_loader = get_dataloader(image_dir,txt_file,batch_size=3,is_training=True, pred_txt_file=None,flag='train')
#batch_size
...
for epoch in range(epochs):
    ...
    #         # print(tmp_loss/(i), mean_f1_score)
    #         # print(f1_score_list)
    #         tmp_statistics_list = []
    #         # start = time.clock()
    ...
    val_loader = get_dataloader(image_dir,txt_file,batch_size=1,is_training=False, pred_txt_file=None,flag='test')
    with torch.no_grad():
