...
save_dir = './models_BP4D/'                                                                                               # directory to save models and log
...
cuda_num = 4                                                                                                       # only support signle gpu card
...
# np.savetxt('./data/train_person.txt',np.array(train_person),fmt='%s')
# np.savetxt('./data/valid_person.txt',np.array(test_person),fmt='%s')
...
# np.savetxt('./data/train_person_withoutBP4D.txt',np.array(train_person),fmt='%s')
...
for epoch in range(epochs):
    ...
    for i in tqdm(range(batches)):
        ...
        if i%print_every == 0:
            ...
            print(tmp_loss/(print_every*i), mean_f1_score)
            print(f1_score_list)
    ...
    # cmp = (pred_test.data).eq(y_test.data).cpu().numpy()
    # correct = cmp.sum()
    # total = len(cmp)*12
    # predicted = pred_test.data.cpu().numpy().tolist()
