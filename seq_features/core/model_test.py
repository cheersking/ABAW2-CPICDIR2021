...
class TwoStreamAUAttentionModelTest(nn.Module):
    def __init__(self, audio_pretrained=False, seq_num=30, encoder_layers=1):
        super(TwoStreamAUAttentionModelTest, self).__init__()
...
class TwoStreamFusionTDNNTest(nn.Module):
    def __init__(self, audio_pretrained=False, seq_num=30, encoder_layers=3, is_fc=False):
        super(TwoStreamFusionTDNNTest, self).__init__()
    ...
    def forward(self, clip, audio, frame_list, batch_size=2, cuda_num=0):
        audio_features = self.audio_model(audio)
